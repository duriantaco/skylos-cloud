---
title: "Finding Dead Code in Flask (69k Stars): Skylos vs Vulture Benchmark"
excerpt: "We ran Skylos and Vulture on the Flask repository. Skylos found all 7 dead items with 12 false positives. Vulture found 6 but produced 260 false positives. Here's the full breakdown with real output."
...
author:
  ...
  type: "Organization"
whyThisExists: "This benchmark shows how framework-heavy Python code changes dead-code accuracy, using Flask because it stresses the exact decorator and fixture patterns that often create false positives."
articleType: "Benchmark"
topic: "Dead Code Detection"
featuredReason: "This benchmark shows what framework awareness changes on a real Flask codebase instead of a toy example."
frameworks: ["Flask"]
methodology:
  - "Cloned the Flask repository and ran both Skylos and Vulture at confidence level 20."
  - "Manually reviewed flagged symbols against the source tree to separate genuinely dead code from framework-driven false positives."
  - "Included the reproduction commands and benchmark script so readers can rerun the comparison."
tags: ["dead code", "python", "sast"]
keywords:
  - flask dead code detection
  - vulture vs skylos benchmark
  - vulture alternative python
  - flask static analysis
  - find unused code python
  - dead code detection benchmark
  - python dead code tool comparison
  - framework aware static analysis python
  - flask code cleanup
  - python dead code detection tools
  - best dead code finder python
