...
excerpt: "Learn how to catch hallucinated imports, phantom calls, hardcoded secrets, and disabled security controls in AI-generated Python code before they reach production."
...
faq:
  - question: "What is a hallucinated import?"
    answer: "A hallucinated import is an import statement for a package or symbol that does not actually exist in your environment or codebase."
  - question: "Why is AI-generated Python code risky even when tests pass?"
    answer: "Tests cover only the paths they exercise. AI-generated code can still hide dead paths, insecure defaults, fake APIs, or missing security controls outside the tested path."
  - question: "Should I review AI-generated code differently from human-written code?"
    answer: "Yes. In addition to normal review, explicitly check whether imports, function calls, libraries, and security controls are real and actually connected."
howToSteps:
  - name: "Scan for nonexistent references"
    text: "Check imports, function calls, and object methods that look valid but do not exist in the real project."
  - name: "Review security-sensitive defaults"
    text: "Look for disabled verification, hardcoded secrets, fake auth checks, and risky convenience code."
  - name: "Add a pre-merge AI review gate"
    text: "Run static analysis and security checks before AI-generated code reaches the default branch."
---
...
1. run a local scan for security and dead code
2. find where LLM integrations actually exist
3. score the AI-specific defenses before the PR merges
...
If you want the pull-request workflow next, go to [Python Security Scanner for GitHub Actions](/use-cases/python-security-github-actions).
...
If you want to compare scanners first, read [Bandit vs Skylos](/compare/bandit-vs-skylos) or [Best Python SAST Tools in 2026](/compare/best-python-sast-tools-2026).

---

## Related

- [How to Review AI-Generated Python Code Before Merge](/blog/review-ai-generated-python-code-before-merge)
