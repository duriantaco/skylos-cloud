...
keyTakeaways:
  - "Dead code in Python usually shows up as unused functions, imports, variables, classes, and unreachable branches."
  - "Framework-heavy apps create false positives, so a generic unused-code scan is rarely enough on its own."
  - "The safest workflow is: detect, review, remove, then gate new dead code in CI."
faq:
  - question: "What is the fastest way to find dead code in Python?"
    answer: "Start with a dedicated unused-code tool, then manually review framework-heavy findings. The fastest safe path combines automated detection with a short human validation pass."
  - question: "Can dead code break production if I remove it?"
    answer: "Yes, if the tool misclassifies dynamically referenced code. That is why framework awareness, import tracing, and review steps matter."
  - question: "Should dead code checks run in CI?"
    answer: "Yes. Once you trust the workflow, CI is the best place to prevent old cleanup work from being undone by new pull requests."
howToSteps:
  - name: "Run a dead-code scan"
    text: "Scan the repository to surface unused functions, imports, classes, and unreachable branches."
  - name: "Review framework-sensitive findings"
    text: "Manually verify decorators, fixtures, routes, and dynamic imports before removal."
  - name: "Gate regressions in CI"
    text: "After the first cleanup pass, run dead-code checks on every pull request so the problem does not quietly return."
---
